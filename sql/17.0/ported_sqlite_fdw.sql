...
--Testcase 760:
ALTER FOREIGN TABLE ft1_a_child OPTIONS (table 'T 1');
--Testcase 761:
ALTER FOREIGN TABLE ft2_a_child OPTIONS (table 'T 1');
--Testcase 762:
...
ALTER EXTENSION sqlite_fdw DROP TEXT SEARCH CONFIGURATION public.custom_search;
...
--Testcase 1002:
CREATE USER MAPPING FOR regress_view_owner_another SERVER sqlite_svr;
--Testcase 1003:
...
EXPLAIN (VERBOSE, COSTS OFF) SELECT * FROM (SELECT * FROM v4 UNION ALL SELECT * FROM v4);
--Testcase 1006:
DROP USER MAPPING FOR regress_view_owner_another SERVER sqlite_svr;
--Testcase 1007:
...
-- PGSpider Extension does not push down when getting sum of bigint column
-- (aggsplit is AGGSPLIT_INITIAL_SERIAL and aggtranstype is INTERNALOID)
--Testcase 800:
set enable_hashagg to false;
--Testcase 210:
explain (verbose, costs off)
...
--Testcase 802:
ALTER FOREIGN TABLE ft1_a_child OPTIONS (SET table 'T 0');
--Testcase 261:
EXPLAIN (VERBOSE, COSTS OFF) EXECUTE st6;
--Testcase 262:
EXECUTE st6;
...
EXPLAIN (VERBOSE, COSTS OFF) EXECUTE st7;
--Testcase 803:
ALTER FOREIGN TABLE ft1_a_child OPTIONS (SET table 'T 1');
...
--Testcase 1004:
SELECT * FROM ft1_a_child ftx(x1,x2,x3,x4,x5,x6,x7,x8) WHERE x1 = 1;    -- ERROR

-- Select some specific columns of foreign table
--Testcase 1005:
SELECT  ftx.x1, ft2.c2 FROM ft1 ftx(x1,x2,x3,x4,x5,x6,x7,x8), ft2 WHERE ftx.x1 = ft2.c1 AND ftx.x1 < 10;  
--Testcase 274:
...
--Testcase 1007:
SELECT ftx1.c1, ftx1.c3, ftx2.c2, ftx2.spdurl FROM ft1 ftx1 JOIN ft2 ftx2 ON (ftx1.c1 = ftx2.c1) AND ftx1.c1 < 10;
--Testcase 1008:
SELECT ftx1.x11, ftx1.x12, ftx1.x18, ftx1.x_spd1, ftx2.x21, ftx2.x22, ftx2.x_spd2 FROM ft1 ftx1(x11,x12,x13,x14,x15,x16,x17,x18,x_spd1) 
LEFT JOIN ft2 ftx2(x21,x22,x23,x24,x25,x26,x27,x28,x_spd2) ON ftx1.x11 = ftx2.x21; -- ERROR

-- Subquery
--Testcase 1009:
SELECT * FROM (SELECT AVG(c2) AS avg_c2 FROM ft1) AS subquery;
--Testcase 1010:
SELECT * FROM ft1 WHERE c2 < (SELECT c1 FROM ft2 WHERE c3 = '00007'); -- ERROR
--Testcase 1011:
SELECT * FROM ft1 ftx(x1,x2,x3,x4,x5,x6,x7,x8) WHERE ftx.x2 IN (SELECT ft2.c1 FROM ft2 WHERE ft2.c2 < 8); -- ERROR
...
-- ===================================================================
--Testcase 558:
create foreign table ft3_a_child (f1 text collate "C", f2 text, f3 varchar(10)) server sqlite_svr options (table 'ft3');
--Testcase 897:
...
--Testcase 282:
EXPLAIN (verbose, costs off)
INSERT INTO ft2_a_child (c1,c2,c3) SELECT c1+1000,c2+100, c3 || c3 FROM ft2_a_child LIMIT 20;
--Testcase 283:
INSERT INTO ft2_a_child (c1,c2,c3) SELECT c1+1000,c2+100, c3 || c3 FROM ft2_a_child LIMIT 20;
--Testcase 284:
...
UPDATE ft2_a_child SET c2 = ft2_a_child.c2 + 500, c3 = ft2_a_child.c3 || '_update9', c7 = DEFAULT
  FROM ft1_a_child WHERE ft1_a_child.c1 = ft2_a_child.c2 AND ft2_a_child.c1 % 10 = 9;                               -- can be pushed down
--Testcase 293:
UPDATE ft2_a_child SET c2 = ft2_a_child.c2 + 500, c3 = ft2_a_child.c3 || '_update9', c7 = DEFAULT
  FROM ft1_a_child WHERE ft1_a_child.c1 = ft2_a_child.c2 AND ft1_a_child.c1 % 10 = 9;
--Testcase 294:
EXPLAIN (verbose, costs off)
...
UPDATE ft2_a_child d SET c2 = CASE WHEN random() >= 0 THEN d.c2 ELSE 0 END
  FROM ft2_a_child AS t WHERE d.c1 = t.c1 AND d.c1 > 1000;
--Testcase 813:
UPDATE ft2_a_child d SET c2 = CASE WHEN random() >= 0 THEN d.c2 ELSE 0 END
  FROM ft2_a_child AS t WHERE d.c1 = t.c1 AND d.c1 > 1000;
...
--Testcase 575:
EXPLAIN (verbose, costs off)
UPDATE ft2_a_child SET c3 = 'bar' WHERE sqlite_fdw_abs(c1) > 2000;            -- can't be pushed down
--Testcase 576:
UPDATE ft2_a_child SET c3 = 'bar' WHERE sqlite_fdw_abs(c1) > 2000;
--Testcase 577:
...
CREATE FOREIGN TABLE loct1_a_child (c1 int OPTIONS (key 'true'))
  SERVER sqlite_svr OPTIONS (table 'loct1_rescan');
--Testcase 1050:
...
CREATE FOREIGN TABLE loct2_a_child (c1 int OPTIONS (key 'true'), c2 text)
  SERVER sqlite_svr OPTIONS (table 'loct2_rescan');
--Testcase 1053:
...
--Testcase 1060:
CREATE FOREIGN TABLE remt2_rescan_a_child (c1 int OPTIONS (key 'true'), c2 text)
  SERVER sqlite_svr OPTIONS (table 'loct2_rescan');
--Testcase 1061:
...
CREATE FOREIGN TABLE foreign_tbl_a_child (a int OPTIONS (key 'true'), b int)
  SERVER sqlite_svr OPTIONS (table 'foreign_tbl');
--Testcase 903:
...
-- --Testcase 615:
-- CREATE TABLE parent_tbl (a int, b int, spdurl text) PARTITION BY RANGE(a);
-- --Testcase 822:
-- ALTER TABLE parent_tbl ATTACH PARTITION foreign_tbl_a_child FOR VALUES FROM (0) TO (100);
...
create foreign table grem1_a_child (
  ...
  server sqlite_svr options(table 'gloc1');
--Testcase 910:
create table grem1 (
  a int,
  b int generated always as (a * 2) stored,
...
--Testcase 405:
EXPLAIN (verbose, costs off)
DELETE FROM rem1;                 -- ERROR due to not supporting foreign delete
--Testcase 696:
...
--Testcase 842:
explain (verbose, costs off)
select * from bar where f1 in (select f1 from foo2_a_child) for share;
--Testcase 843:
select * from bar where f1 in (select f1 from foo2_a_child) for share;
...
--Testcase 935:
CREATE FOREIGN TABLE ftable_a_child ( x int OPTIONS (key 'true') ) SERVER sqlite_svr OPTIONS ( table 'batch_table', batch_size '10' );
--Testcase 980:
...
--Testcase 949:
CREATE FOREIGN TABLE ftable_a_child ( x int ) SERVER sqlite_svr OPTIONS ( table 'batch_table', batch_size '1' );
--Testcase 986:
CREATE TABLE ftable ( x int, spdurl text) PARTITION BY LIST (spdurl);
--Testcase 987:
CREATE FOREIGN TABLE ftable_a PARTITION OF ftable FOR VALUES IN ('/node1/') SERVER spdsrv;
