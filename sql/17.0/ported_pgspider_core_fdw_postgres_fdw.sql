...
--Testcase 6:
CREATE USER MAPPING FOR CURRENT_USER SERVER pgspider_ext_svr;
--Testcase 7:
...
--Testcase 21:
ALTER TABLE ft1__postgres_srv__0 DROP COLUMN c0;

--Testcase 1056:
CREATE FOREIGN TABLE ft1__postgres_srv__0_child1 PARTITION OF ft1 FOR VALUES IN ('/postgres_srv/') SERVER pgspider_ext_svr OPTIONS(child_name 'ft1__postgres_srv__0');
...
--Testcase 1057:
CREATE FOREIGN TABLE ft2__postgres_srv__0_child1 PARTITION OF ft2 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'ft2__postgres_srv__0');
...
--Testcase 1058:
CREATE FOREIGN TABLE ft4__postgres_srv__0_child1 PARTITION OF ft4 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'ft4__postgres_srv__0');
...
--Testcase 1059:
CREATE FOREIGN TABLE ft5__postgres_srv__0_child1 PARTITION OF ft5 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'ft5__postgres_srv__0');
...
--Testcase 1060:
CREATE FOREIGN TABLE ft6__postgres_srv2__0_child1 PARTITION OF ft6 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'ft6__postgres_srv2__0');
...
--  ALTER SERVER pgspider_ext_svr OPTIONS (
...
--  ALTER SERVER pgspider_ext_svr OPTIONS (ADD extensions 'foo; bar');

-- OK but gets a warning
-- ALTER SERVER pgspider_ext_svr OPTIONS (ADD extensions 'foo, bar');
-- ALTER SERVER pgspider_ext_svr OPTIONS (DROP extensions);

-- Skip, pgspider_core_fdw does not support ssl
-- Attempt to add a valid option that's not allowed in a user mapping
--ALTER USER MAPPING FOR public SERVER pgspider_ext_svr
...
-- --Testcase 35:
-- ALTER TABLE ft1 ALTER COLUMN c1 OPTIONS (column_name 'C 1');
--Testcase 36:
ALTER FOREIGN TABLE ft1__postgres_srv__0 ALTER COLUMN c1 OPTIONS (column_name 'C 1');
-- --Testcase 37:
-- ALTER TABLE ft2 ALTER COLUMN c1 OPTIONS (column_name 'C 1');
--Testcase 38:
...
-- ===================================================================
--Testcase 1323:
CREATE PUBLICATION testpub_ftbl FOR TABLE ft1__postgres_srv__0_child1;  -- should fail
...
--Testcase 77:
CREATE TABLE ft_empty (c1 int NOT NULL, c2 text, __spd_url text)
   PARTITION BY LIST (__spd_url);
--Testcase 78:
CREATE FOREIGN TABLE ft_empty__postgres_srv__0 (c1 int NOT NULL, c2 text)
  SERVER postgres_srv OPTIONS (table_name 'ft_empty');
--Testcase 1063:
CREATE FOREIGN TABLE ft_empty__postgres_srv__0_child1 PARTITION OF ft_empty FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'ft_empty__postgres_srv__0');
--Testcase 79:
...
--Testcase 1466:
INSERT INTO ft1 (c1) VALUES (1); -- ERROR due to the missing _spd_url column during the insert.
--Testcase 1467:
INSERT INTO ft1 (c1, __spd_url) VALUES (1, '/postgres_srv/'); -- ERROR due to not supporting foreign insert.
--Testcase 1468:
...
--Testcase 113:
ALTER EXTENSION pgspider_ext ADD FUNCTION postgres_fdw_abs(int);
--Testcase 114:
ALTER EXTENSION pgspider_ext ADD OPERATOR === (int, int);
--Testcase 115:
...
select dblink_connect('dbname=postdb host=127.0.0.1
  port=15432 user=postgres password=postgres');
-- Re-nable to pushdown aggregate
SET enable_partitionwise_aggregate TO on;
SET parallel_leader_participation = 'off';
...
-- SEMI JOIN
-- pgspider_core_fdw does not support pushing down SEMI-JOIN
--Testcase 185:
EXPLAIN (VERBOSE, COSTS OFF)
...
--Testcase 220:
CREATE USER MAPPING FOR regress_view_owner SERVER pgspider_ext_svr;
...
--Testcase 1328:
CREATE USER MAPPING FOR regress_view_owner_another SERVER pgspider_ext_svr;
--Testcase 1329:
EXPLAIN (VERBOSE, COSTS OFF) SELECT * FROM v4;
--Testcase 1330:
EXPLAIN (VERBOSE, COSTS OFF) SELECT * FROM (SELECT * FROM v4 UNION ALL SELECT * FROM v4);
--Testcase 1331:
DROP USER MAPPING FOR regress_view_owner_another SERVER pgspider_ext_svr;
--Testcase 1332:
...
--Testcase 293:
alter extension pgspider_ext add function least_accum(anyelement, variadic anyarray);
--Testcase 294:
alter extension pgspider_ext add aggregate least_agg(variadic items anyarray);
--Testcase 295:
...
--Testcase 298:
alter extension pgspider_ext drop function least_accum(anyelement, variadic anyarray);
--Testcase 299:
alter extension pgspider_ext drop aggregate least_agg(variadic items anyarray);
--Testcase 300:
alter server postgres_srv options (set extensions 'postgres_fdw');
...
-- Add into extension
--Testcase 312:
alter extension pgspider_ext add operator class my_op_class using btree;
--Testcase 313:
alter extension pgspider_ext add function my_op_cmp(a int, b int);
--Testcase 314:
alter extension pgspider_ext add operator family my_op_family using btree;
--Testcase 315:
alter extension pgspider_ext add operator public.<^(int, int);
--Testcase 316:
alter extension pgspider_ext add operator public.=^(int, int);
--Testcase 317:
alter extension pgspider_ext add operator public.>^(int, int);
...
-- alter server pgspider_ext_svr options (add fdw_tuple_cost '0.5');
--Testcase 319:
explain (verbose, costs off)
select array_agg(c1 order by c1 using operator(public.<^)) from ft2 where c2 = 6 and c1 < 100 group by c2;
--Testcase 320:
select array_agg(c1 order by c1 using operator(public.<^)) from ft2 where c2 = 6 and c1 < 100 group by c2;
-- alter server pgspider_ext_svr options (drop fdw_tuple_cost);
...
--Testcase 321:
alter extension pgspider_ext drop operator class my_op_class using btree;
--Testcase 322:
alter extension pgspider_ext drop function my_op_cmp(a int, b int);
--Testcase 323:
alter extension pgspider_ext drop operator family my_op_family using btree;
--Testcase 324:
alter extension pgspider_ext drop operator public.<^(int, int);
--Testcase 325:
alter extension pgspider_ext drop operator public.=^(int, int);
--Testcase 326:
alter extension pgspider_ext drop operator public.>^(int, int);
...
--Testcase 413:
SELECT * FROM ft1 t1 WHERE t1.tableoid = 'ft1__postgres_srv__0_child1'::regclass LIMIT 1;
--Testcase 414:
EXPLAIN (VERBOSE, COSTS OFF)
...
-- This test case for postgres version 14.0
-- -- ===================================================================
-- -- REINDEX
-- -- ===================================================================
...
-- ===================================================================
-- --Testcase 430:
-- ALTER TABLE ft1 ALTER COLUMN c8 TYPE int;
--Testcase 431:
...
--Testcase 1369:
SELECT ftx.x1, ft2.c2 FROM ft1 ftx(x1,x2,x3,x4,x5,x6,x7,x8), ft2 
  WHERE ftx.x1 = ft2.c1 AND ftx.x1 < 10;
--Testcase 433:
...
-- Join query 
--Testcase 1370:
SELECT * FROM ft1 ftx1 JOIN ft2 ftx2 ON (ftx1.c1 = ftx2.c1); -- ERROR
--Testcase 1371:
SELECT ftx1.c1, ftx1.c3, ftx2.c2, ftx2.__spd_url 
  FROM ft1 ftx1 
  JOIN ft2 ftx2 
  ON (ftx1.c1 = ftx2.c1) 
  WHERE ftx2.c1 < 10;
...
SELECT ftx1.x11, ftx1.x12, ftx1.x18, ftx1.x_spd1, ftx2.x21, ftx2.x22, ftx2.x_spd2 
  FROM ft1 ftx1(x11,x12,x13,x14,x15,x16,x17,x18,x_spd1) 
  LEFT JOIN ft2 ftx2(x21,x22,x23,x24,x25,x26,x27,x28,x_spd2)
  ON ftx1.x11 = ftx2.x21; -- ERROR

-- Subquery
--Testcase 1373:
SELECT * FROM (SELECT AVG(c2) AS avg_c2 FROM ft1) AS subquery;
--Testcase 1374:
SELECT * FROM ft1 WHERE c2 < (SELECT c1 FROM ft1  WHERE c3 = '00010'); -- ERROR
--Testcase 1375:
SELECT * FROM ft1 WHERE ft1.c2 IN (SELECT ft2.c1 FROM ft1 ft2 WHERE ft2.c2 < 8); -- ERROR

-- Aggregate function
--Testcase 1376:
SELECT COUNT(ft1.c3), SUM(ft1.c2), AVG(ft1.c1) FROM ft1 WHERE ft1.c1 > 2;
--Testcase 435:
SELECT sum(c2), array_agg(c8) FROM ft1 GROUP BY c8; -- ERROR
-- ANALYZE ft1; -- ERROR
ALTER TABLE ft1 ALTER COLUMN c8 TYPE user_enum;
...
BEGIN;
--Testcase 1180:
DECLARE c1 CURSOR FOR SELECT count(*) FROM ft1 t1;
--Testcase 1181:
SAVEPOINT s;
--Testcase 1182:
ERROR OUT;
...
--Testcase 1204:
-- On pgspider extension does not multi thread. -> this case will success
--Testcase 1334:
...
-- ===================================================================
--Testcase 443:
create table ft3 (f1 text collate "C", f2 text, f3 varchar(10), __spd_url text)
   PARTITION BY LIST (__spd_url);
--Testcase 444:
...
--Testcase 1064:
CREATE FOREIGN TABLE ft3__postgres_srv__0_child1 PARTITION OF ft3 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'ft3__postgres_srv__0');
...
-- pgspider_ext does not support to push down join clause
-- ===================================================================
--Testcase 1374:
EXPLAIN (verbose, costs off)
SELECT ft2.c1, ft2.c2, ft2.c3, ft2.c4, ft2.c5, ft2.c6, ft2.c7, ft2.c8, ft4.c1, ft4.c2, ft4.c3 FROM ft2 INNER JOIN ft4 ON ft2.c2 = ft4.c1
  WHERE ft2.c1 > 900
...
--Testcase 1376:
EXPLAIN (verbose, costs off)
SELECT ft2.c1, ft2.c2, ft2.c3, ft2.c4, ft2.c5, ft2.c6, ft2.c7, ft2.c8, ft4.c1, ft4.c2, ft4.c3 FROM ft2 INNER JOIN
...
--Testcase 1378:
EXPLAIN (verbose, costs off)
SELECT ft2.c1, ft2.c2, ft2.c3, ft2.c4, ft2.c5, ft2.c6, ft2.c7, ft2.c8, ft4.c1, ft4.c2, ft4.c3 FROM ft2 LEFT JOIN
  (SELECT * FROM ft4 WHERE
  EXISTS (SELECT 1 FROM ft5 WHERE ft4.c1 = ft5.c1)) ft4
  ON ft2.c2 = ft4.c1
  WHERE ft2.c1 > 900
...
--Testcase 1382:
EXPLAIN (verbose, costs off)
SELECT ft2.c1, ft2.c2, ft2.c3, ft2.c4, ft2.c5, ft2.c6, ft2.c7, ft2.c8 FROM ft2 WHERE
  c1 = ANY (
...
--Testcase 1384:
EXPLAIN (verbose, costs off)
SELECT c1, c2, c3, c4, c5, c6, c7, c8 FROM ft2 ftupper WHERE
   EXISTS (
	SELECT c1 FROM ft2 WHERE
...
-- Due to an existing bug of pgspider_core_fdw, this test case will return error.
-- Temporally comment it out
-- EXPLAIN (verbose, costs off)
-- 	SELECT ft2.c1, ft2.c2, ft2.c3, ft2.c4, ft2.c5, ft2.c6, ft2.c7, ft2.c8, ft4.c1, ft4.c2, ft4.c3 FROM ft2 INNER JOIN
...
-- change (bug #17889)
--Testcase 1468:
CREATE TABLE loct1 (c1 int, __spd_url text) PARTITION BY LIST (__spd_url);
--Testcase 1469:
CREATE FOREIGN TABLE loct1__postgres_srv__0 (c1 int) SERVER postgres_srv OPTIONS (table_name 'loct1_rescan');
--Testcase 1470:
CREATE FOREIGN TABLE loct1__postgres_srv__0_child1 PARTITION OF loct1 FOR VALUES IN ('/pgspider_srv_1/') 
  SERVER pgspider_ext_svr OPTIONS(child_name 'loct1__postgres_srv__0');
--Testcase 1471:
CREATE TABLE remt2 (c1 int, c2 text, __spd_url text) PARTITION BY LIST (__spd_url);
--Testcase 1472:
CREATE FOREIGN TABLE remt2__postgres_srv__0 (c1 int,  c2 text) SERVER postgres_srv OPTIONS (table_name 'loct2_rescan');
--Testcase 1473:
CREATE FOREIGN TABLE remt2__postgres_srv__0_child1 PARTITION OF remt2 FOR VALUES IN ('/pgspider_srv_1/') 
  SERVER pgspider_ext_svr OPTIONS(child_name 'remt2__postgres_srv__0');
-- ANALYZE loct1;
-- ANALYZE remt2;
...
--Testcase 557:
CREATE TABLE foreign_tbl (a int, b int, __spd_url text)
   PARTITION BY LIST (__spd_url);
--Testcase 558:
CREATE FOREIGN TABLE foreign_tbl__postgres_srv__0 (a int, b int)
  SERVER postgres_srv OPTIONS(table_name 'base_tbl');
--Testcase 1065:
CREATE FOREIGN TABLE foreign_tbl__postgres_srv__0_child1 PARTITION OF foreign_tbl FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'foreign_tbl__postgres_srv__0');
...
-- ===================================================================
--Testcase 577:
create table rem1 (f1 serial, f2 text, __spd_url text)
   PARTITION BY LIST (__spd_url);
--Testcase 578:
create foreign table rem1__postgres_srv__0 (f1 serial, f2 text)
  server postgres_srv options(table_name 'loc1_1');
--Testcase 1066:
CREATE FOREIGN TABLE rem1__postgres_srv__0_child1 PARTITION OF rem1 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'rem1__postgres_srv__0');
...
--Testcase 1067:
CREATE FOREIGN TABLE grem1__postgres_srv__0_child1 PARTITION OF grem1 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'grem1__postgres_srv__0');
--Testcase 588:
explain (verbose, costs off)
...
--Testcase 706:
CREATE TABLE b (aa TEXT, bb TEXT, __spd_url TEXT)
  PARTITION BY LIST (__spd_url);
--Testcase 707:
CREATE FOREIGN TABLE b__postgres_srv__0 (bb TEXT) INHERITS (a)
  SERVER postgres_srv OPTIONS (table_name 'loct_1');
--Testcase 1077:
CREATE FOREIGN TABLE b__postgres_srv__0_child1 PARTITION OF b FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'b__postgres_srv__0');
...
--Testcase 736:
create table foo2 (f1 int, f2 int, f3 int, __spd_url text)
  PARTITION BY LIST (__spd_url);
--Testcase 737:
create foreign table foo2__postgres_srv__0 (f3 int) inherits (foo)
  server postgres_srv options (table_name 'loct1_1');
--Testcase 1078:
CREATE FOREIGN TABLE foo2__postgres_srv__0_child1 PARTITION OF foo2 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'foo2__postgres_srv__0');

--Testcase 738:
create table bar (f1 int, f2 int);
--Testcase 739:
create table bar2 (f1 int, f2 int, f3 int, __spd_url text)
  PARTITION BY LIST (__spd_url);
--Testcase 740:
create foreign table bar2__postgres_srv__0 (f3 int) inherits (bar)
  server postgres_srv options (table_name 'loct2_1');
--Testcase 1079:
CREATE FOREIGN TABLE bar2__postgres_srv__0_child1 PARTITION OF bar2 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'bar2__postgres_srv__0');
...
--Testcase 757:
create table foo2child (f1 int, f2 int, f3 int, __spd_url text)
  PARTITION BY LIST (__spd_url);
--Testcase 758:
create foreign table foo2child__postgres_srv__0 (f1 int, f2 int, f3 int)
  server postgres_srv options (table_name 'loct4');
--Testcase 1080:
CREATE FOREIGN TABLE foo2child__postgres_srv__0_child1 PARTITION OF foo2child FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'foo2child__postgres_srv__0');
...
SET enable_nestloop to false;
--Testcase 777:
alter foreign table foo2__postgres_srv__0 options (use_remote_estimate 'true');
analyze foo;
-- inner join; expressions in the clauses appear in the equivalence class list
--Testcase 778:
explain (verbose, costs off)
...
--Testcase 799:
create table remt1 (a int, b text, __spd_url text)
  PARTITION BY LIST (__spd_url);
--Testcase 800:
create foreign table remt1__postgres_srv__0 (a int, b text)
  server postgres_srv options (table_name 'loct1_2');
--Testcase 1082:
CREATE FOREIGN TABLE remt1__postgres_srv__0_child1 PARTITION OF remt1 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'remt1__postgres_srv__0');

--Testcase 801:
create table remt2 (a int, b text, __spd_url text)
  PARTITION BY LIST (__spd_url);
--Testcase 802:
create foreign table remt2__postgres_srv__0 (a int, b text)
  server postgres_srv options (table_name 'loct2_2');
--Testcase 1083:
CREATE FOREIGN TABLE remt2__postgres_srv__0_child1 PARTITION OF remt2 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'remt2__postgres_srv__0');
...
--Testcase 817:
create table itrtest (a int, b text, __spd_url text)
  PARTITION BY LIST (__spd_url);
--Testcase 818:
create foreign table itrtest__postgres_srv__0 (a int, b text)
  server postgres_srv options (table_name 'itrtest');
--Testcase 1084:
CREATE FOREIGN TABLE itrtest__postgres_srv__0_child1 PARTITION OF itrtest FOR VALUES IN ('/postgres_srv/') SERVER pgspider_ext_svr OPTIONS(child_name 'itrtest__postgres_srv__0');
...
--Testcase 820:
create table remp1 (a int check (a in (1)), b text, __spd_url text)
  PARTITION BY LIST (__spd_url);
--Testcase 821:
create foreign table remp1__postgres_srv__0 (a int check (a in (1)), b text)
  server postgres_srv options (table_name 'remp1');
--Testcase 1085:
CREATE FOREIGN TABLE remp1__postgres_srv__0_child1 PARTITION OF remp1 FOR VALUES IN ('/postgres_srv/') SERVER pgspider_ext_svr OPTIONS(child_name 'remp1__postgres_srv__0');
...
--Testcase 823:
create table remp2 (b text, a int check (a in (2)), __spd_url text)
  PARTITION BY LIST (__spd_url);
--Testcase 824:
create foreign table remp2__postgres_srv__0 (b text, a int check (a in (2)))
  server postgres_srv options (table_name 'remp2');
--Testcase 1086:
CREATE FOREIGN TABLE remp2__postgres_srv__0_child1 PARTITION OF remp2 FOR VALUES IN ('/postgres_srv/') SERVER pgspider_ext_svr OPTIONS(child_name 'remp2__postgres_srv__0');
...
--Testcase 859:
create table utrtest (a int, b text, __spd_url text)
PARTITION BY LIST (__spd_url);
--Testcase 860:
create foreign table utrtest__postgres_srv__0 (a int, b text)
  server postgres_srv options (table_name 'utrtest');
--Testcase 1096:
create foreign table utrtest_child partition of utrtest for values in ('/postgres_srv/') 
server pgspider_ext_svr OPTIONS(child_name 'utrtest__postgres_srv__0');
--Testcase 861:
...
--Testcase 862:
create table remp (a int check (a in (1)), b text, __spd_url text)
  PARTITION BY LIST (__spd_url);
--Testcase 863:
create foreign table remp__postgres_srv__0 (a int check (a in (1)), b text)
  server postgres_srv options (table_name 'remp');
--Testcase 1087:
CREATE FOREIGN TABLE remp__postgres_srv__0_child1 PARTITION OF remp FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'remp__postgres_srv__0');
...
--Testcase 1088:
CREATE FOREIGN TABLE locp__postgres_srv__0_child1 PARTITION OF locp FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'locp__postgres_srv__0');
...
--Testcase 910:
create table ctrtest (a int, b text, __spd_url text)
  PARTITION BY LIST (__spd_url);
--Testcase 911:
create foreign table ctrtest__postgres_srv__0 (a int, b text)
  server postgres_srv options (table_name 'ctrtest');
--Testcase 1089:
CREATE FOREIGN TABLE ctrtest__postgres_srv__0_child1 PARTITION OF ctrtest FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'ctrtest__postgres_srv__0');
...
--Testcase 1090:
CREATE FOREIGN TABLE remp1__postgres_srv__0_child1 PARTITION OF remp1 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'remp1__postgres_srv__0');
...
--Testcase 1091:
CREATE FOREIGN TABLE remp2__postgres_srv__0_child1 PARTITION OF remp2 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'remp2__postgres_srv__0');
...
--Testcase 1335:
delete from remp1__postgres_srv__0;
--Testcase 1336:
delete from remp2__postgres_srv__0;
...
--Testcase 926:
create table rem2 (f1 int, f2 text, __spd_url text)
  PARTITION BY LIST (__spd_url);
--Testcase 927:
create foreign table rem2__postgres_srv__0 (f1 int, f2 text)
  server postgres_srv options(table_name 'loc2_1');
--Testcase 1092:
CREATE FOREIGN TABLE rem2__postgres_srv__0_child1 PARTITION OF rem2 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'rem2__postgres_srv__0');
...
-- Test check constraints
--Testcase 930:
alter foreign table rem2__postgres_srv__0 add constraint rem2_f1positive check (f1 >= 0);
...
--Testcase 932:
alter foreign table rem2__postgres_srv__0 drop constraint rem2_f1positive;
...
begin;
--Testcase 968:
create table rem3 (f1 int, f2 text, __spd_url text)
	PARTITION BY LIST (__spd_url);
--Testcase 969:
create foreign table rem3__postgres_srv__0 (f1 int, f2 text)
	server postgres_srv options(table_name 'loc3_1');
--Testcase 1093:
CREATE FOREIGN TABLE rem3__postgres_srv__0_child1 PARTITION OF rem3 FOR VALUES IN ('/pgspider_srv_1/') SERVER pgspider_ext_svr OPTIONS(child_name 'rem3__postgres_srv__0');
...
-- Test check constraints
--Testcase 1338:
SELECT dblink_exec('alter table loc2_1 add constraint loc2_f1positive check (f1 >= 0)');
...
--Testcase 1339:
SELECT dblink_exec('alter table loc2_1 drop constraint loc2_f1positive');
...
--Testcase 1347:
SELECT dblink_exec('alter table loc2_1 drop column f1;');
--Testcase 1348:
SELECT dblink_exec('alter table loc2_1 drop column f2;');
...
-- Thest test case for postgres version 14.0
-- -- ===================================================================
-- -- test for TRUNCATE
-- -- ===================================================================
-- --Testcase 973:
...
-- These test case for feature of postgres 14.0
-- -- ===================================================================
-- -- batch insert
-- -- ===================================================================
...
/* PGSpider Extension does not support these following options */
-- -- ===================================================================
...
CREATE TABLE prem1 (f1 int, f2 text, __spd_url text)
  PARTITION BY LIST (__spd_url);
--Testcase 522:
CREATE FOREIGN TABLE prem1_child PARTITION OF prem1 FOR VALUES IN ('/node1/') SERVER pgspider_ext_svr OPTIONS(child_name 'prem1_orig');
...
CREATE TABLE prem2 (f1 int, f2 text, __spd_url text)
  PARTITION BY LIST (__spd_url);
--Testcase 526:
CREATE FOREIGN TABLE prem2_child PARTITION OF prem2 FOR VALUES IN ('/node1/') SERVER pgspider_ext_svr OPTIONS(child_name 'prem2_orig');
...
BEGIN;
--Testcase 1350:
INSERT INTO prem1_orig VALUES (105, 'test1');
--Testcase 1351:
INSERT INTO prem2_orig VALUES (205, 'test2');
ABORT;
--Testcase 1352:
SELECT * FROM prem1;
--Testcase 1353:
...
CREATE FOREIGN TABLE analyze_ftable_orig (id int, a text, b bigint)
       SERVER postgres_srv OPTIONS (table_name 'analyze_table');
...
--Testcase 1362:
CREATE FOREIGN TABLE analyze_ftable_child PARTITION OF analyze_ftable FOR VALUES IN ('/node1/') SERVER pgspider_ext_svr OPTIONS(child_name 'analyze_ftable_orig');

--Testcase 1363:
INSERT INTO analyze_ftable_orig (SELECT x FROM generate_series(1,1000) x);
ANALYZE analyze_ftable_orig;
...
-- cleanup
--Testcase 1364:
DELETE FROM analyze_ftable_orig;
--Testcase 1365:
DROP FOREIGN TABLE analyze_ftable_orig;
--Testcase 1366:
DROP FOREIGN TABLE analyze_ftable_child;
--Testcase 1367:
...
--Testcase 1419:
DROP SERVER pgspider_ext_svr CASCADE;
--Testcase 1420:
DROP SERVER postgres_srv CASCADE;
--Testcase 1421:
DROP SERVER postgres_srv2 CASCADE;
--Testcase 1422:
DROP SERVER postgres_srv3 CASCADE;
--Testcase 1423:
