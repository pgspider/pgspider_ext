...
CREATE SERVER griddb_svr FOREIGN DATA WRAPPER griddb_fdw
	OPTIONS (host :GRIDDB_HOST, port :GRIDDB_PORT, clustername :GRIDDB_CLUSTERNAME);
...
\c -
-- re-set flag after reconnect database
-- Enable to pushdown aggregate
SET enable_partitionwise_aggregate TO on;
-- Turn off leader node participation to avoid duplicate data error when executing
-- parallel query
...
-- change (bug #17889)
--Testcase 1226:
CREATE FOREIGN TABLE loct1_a_child (c1 int OPTIONS (rowkey 'true'))
  SERVER griddb_svr OPTIONS (table_name 'loct1_rescan');
--Testcase 1227:
...
--Testcase 1229:
CREATE FOREIGN TABLE loct2_a_child (c1 int OPTIONS (rowkey 'true'), c2 text)
  SERVER griddb_svr OPTIONS (table_name 'loct2_rescan');
--Testcase 1230:
...
--Testcase 1237:
CREATE FOREIGN TABLE remt2_rescan_a_child (c1 int OPTIONS (rowkey 'true'), c2 text)
  SERVER griddb_svr OPTIONS (table_name 'loct2_rescan');
--Testcase 1238:
