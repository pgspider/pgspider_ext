...
ALTER EXTENSION dynamodb_fdw ADD TEXT SEARCH CONFIGURATION public.custom_search;
...
--Testcase 802:
EXPLAIN (VERBOSE, COSTS OFF)
SELECT * FROM local_tbl LEFT JOIN (SELECT ft1.* FROM ft1 INNER JOIN ft2 ON (ft1.c1 = ft2.c1 AND ft1.c1 < 100 AND (ft1.c1 - dynamodb_fdw_abs(ft2.c2)) = 0)) ss ON (local_tbl.c3 = ss.c3) ORDER BY local_tbl.c1 FOR UPDATE OF local_tbl;
...
CREATE FOREIGN TABLE loct1_a_child (c1 int)
  SERVER dynamodb_server OPTIONS (table_name 'loct1_rescan', partition_key 'c1');
--Testcase 870:
...
CREATE FOREIGN TABLE loct2_a_child (c1 int, c2 text)
  SERVER dynamodb_server OPTIONS (table_name 'loct2_rescan', partition_key 'c1');
--Testcase 873:
...
--Testcase 880:
CREATE FOREIGN TABLE remt2_rescan_a_child (c1 int, c2 text)
  SERVER dynamodb_server OPTIONS (table_name 'loct2_rescan', partition_key 'c1');
--Testcase 881:
...
-- PGSpider Extension only support partition list
