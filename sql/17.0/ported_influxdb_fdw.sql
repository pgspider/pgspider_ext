...
-- ===================================================================
\set ECHO none
\ir sql/parameters/influxdb_parameters.conf
\set ECHO all
...
--Testcase 635:
ALTER FOREIGN TABLE ft1_a_child OPTIONS (table 'T1', tags 'c3');
--Testcase 636:
ALTER FOREIGN TABLE ft2_a_child OPTIONS (table 'T1', tags 'c3');
--Testcase 637:
...
--Testcase 30:
EXPLAIN (COSTS OFF) SELECT * FROM ft1 ORDER BY c1, c3 OFFSET 100 LIMIT 10;
--Testcase 31:
SELECT * FROM ft1 ORDER BY c1, c3 OFFSET 100 LIMIT 10;
-- single table with alias - also test that tableoid sort is not pushed to remote side
--Testcase 32:
EXPLAIN (VERBOSE, COSTS OFF) SELECT * FROM ft1 t1 ORDER BY t1.c1, t1.c3, t1.tableoid OFFSET 100 LIMIT 10;
--Testcase 33:
SELECT * FROM ft1 t1 ORDER BY t1.c1, t1.c3, t1.tableoid OFFSET 100 LIMIT 10;
-- whole-row reference
--Testcase 34:
EXPLAIN (VERBOSE, COSTS OFF) SELECT t1 FROM ft1 t1 ORDER BY t1.c1, t1.c3 OFFSET 100 LIMIT 10;
--Testcase 35:
SELECT t1 FROM ft1 t1 ORDER BY t1.c1, t1.c3 OFFSET 100 LIMIT 10;
-- empty result
--Testcase 36:
...
\c -
-- reset-flag after reconnect database
-- Enable to pushdown aggregate
SET enable_partitionwise_aggregate TO on;
SET parallel_leader_participation = 'off';
...
--Testcase 178:
CREATE FOREIGN TABLE local_tbl_a_child (c1 int NOT NULL, c2 int NOT NULL, c3 text) SERVER influxdb_svr OPTIONS (table 'local_tbl');
--Testcase 659:
CREATE TABLE local_tbl (c1 int NOT NULL, c2 int NOT NULL, c3 text, spdurl text) PARTITION BY LIST (spdurl);
--Testcase 660:
...
--Testcase 801:
CREATE USER MAPPING FOR regress_view_owner_another SERVER influxdb_svr OPTIONS (user :INFLUXDB_USER, password :INFLUXDB_PASS);
--Testcase 802:
...
-- InfluxDB not support user-defined operator
--Testcase 767:
explain (verbose, costs off)
select * from ft2 order by c1 using operator(public.<^);
...
--Testcase 690:
ALTER FOREIGN TABLE ft1_a_child OPTIONS (SET table 'T0');
--Testcase 336:
EXPLAIN (VERBOSE, COSTS OFF) EXECUTE st6;
--Testcase 337:
EXECUTE st6;
...
DELETE FROM "S 1"."T 0";
--Testcase 691:
ALTER FOREIGN TABLE ft1_a_child OPTIONS (SET table 'T1');
...
--Testcase 345:
SELECT * FROM ft1 t1 WHERE t1.tableoid = 'ft1_a'::regclass ORDER BY c1 LIMIT 1;
--Testcase 346:
EXPLAIN (VERBOSE, COSTS OFF)
...
-- ===================================================================
--Testcase 360:
create foreign table loct3_a_child (f1 text collate "C", f2 text, f3 varchar(10)) server influxdb_svr options (table 'loct3');
--Testcase 694:
create table loct3 (f1 text collate "C", f2 text, f3 varchar(10), spdurl text) PARTITION BY LIST (spdurl);
--Testcase 695:
...
-- change (bug #17889)
--Testcase 860:
CREATE FOREIGN TABLE loct1_rescan_a_child (c1 int) SERVER influxdb_svr OPTIONS (table 'loct1_rescan');
--Testcase 861:
CREATE TABLE loct1_rescan (c1 int, spdurl text) PARTITION BY LIST (spdurl);
--Testcase 862:
CREATE FOREIGN TABLE loct1_rescan_a PARTITION OF loct1_rescan FOR VALUES IN ('/node1/') SERVER spdsrv;
--Testcase 863:
CREATE FOREIGN TABLE loct2_rescan_a_child (c1 int, c2 text) SERVER influxdb_svr OPTIONS (table 'loct2_rescan');
--Testcase 864:
CREATE TABLE loct2_rescan (c1 int, c2 text, spdurl text) PARTITION BY LIST (spdurl);
--Testcase 865:
CREATE FOREIGN TABLE loct2_rescan_a PARTITION OF loct2_rescan FOR VALUES IN ('/node1/') SERVER spdsrv;
--Testcase 866:
INSERT INTO loct1_rescan (c1, spdurl) VALUES (1001, '/node1/');
--Testcase 867:
INSERT INTO loct1_rescan (c1, spdurl) VALUES (1001, '/node1/');
--Testcase 868:
INSERT INTO loct2_rescan SELECT id, to_char(id, 'FM0000'), '/node1/' FROM generate_series(1, 1000) id;
--Testcase 869:
INSERT INTO loct2_rescan (c1, c2, spdurl) VALUES (1001, 'foo', '/node1/');
--Testcase 870:
INSERT INTO loct2_rescan (c1, c2, spdurl) VALUES (1002, 'bar', '/node1/');
--Testcase 871:
CREATE FOREIGN TABLE remt2_rescan_a_child (c1 int, c2 text) SERVER influxdb_svr OPTIONS (table 'loct2_rescan');
--Testcase 872:
CREATE TABLE remt2_rescan (c1 int, c2 text, spdurl text) PARTITION BY LIST (spdurl);
--Testcase 873:
...
UPDATE remt2_rescan SET c2 = remt2_rescan.c2 || remt2_rescan.c2 FROM loct1_rescan WHERE loct1_rescan.c1 = remt2_rescan.c1;
--Testcase 876:
SELECT remt2_rescan.c1, remt2_rescan.c2 FROM loct1_rescan, remt2_rescan WHERE loct1_rescan.c1 = remt2_rescan.c1;
...
--Testcase 878:
DROP TABLE loct1_rescan;
--Testcase 879:
DROP TABLE loct2_rescan;
--Testcase 880:
...
--Testcase 428:
CREATE FOREIGN TABLE base_tbl_a_child (a int, b int) SERVER influxdb_svr OPTIONS (table 'base_tbl');
--Testcase 709:
CREATE TABLE base_tbl (a int, b int, spdurl text) PARTITION BY LIST (spdurl);
--Testcase 710:
CREATE FOREIGN TABLE base_tbl_a PARTITION OF base_tbl FOR VALUES IN ('/node1/') SERVER spdsrv;
--ALTER FOREIGN TABLE base_tbl SET (autovacuum_enabled = 'false');
--Testcase 429:
CREATE TRIGGER row_before_insupd_trigger BEFORE INSERT OR UPDATE ON base_tbl_a_child FOR EACH ROW EXECUTE PROCEDURE row_before_insupd_trigfunc();
--Testcase 430:
CREATE FOREIGN TABLE foreign_tbl_a_child (a int, b int)
...
--Testcase 431:
CREATE VIEW rw_view AS SELECT * FROM base_tbl_a_child
...
--Testcase 439:
DROP TRIGGER row_before_insupd_trigger ON base_tbl_a_child;
--Testcase 440:
DROP FOREIGN TABLE base_tbl_a_child CASCADE;
--Testcase 714:
...
-- --Testcase 441:
-- CREATE FOREIGN TABLE child_tbl_a_child (a int, b int) SERVER influxdb_svr OPTIONS (table 'child_tbl');
-- CREATE TABLE child_tbl (a int, b int, spdurl text) PARTITION BY LIST (spdurl);
-- CREATE FOREIGN TABLE child_tbl_a PARTITION OF child_tbl FOR VALUES IN ('/node1/') SERVER spdsrv;
-- --ALTER FOREIGN TABLE child_tbl SET (autovacuum_enabled = 'false');
-- --Testcase 442:
-- CREATE TRIGGER row_before_insupd_trigger BEFORE INSERT OR UPDATE ON child_tbl_a_child FOR EACH ROW EXECUTE PROCEDURE row_before_insupd_trigfunc();
-- --Testcase 443:
-- CREATE FOREIGN TABLE foreign_tbl_a_child (a int, b int)
...
-- ===================================================================
--Testcase 457:
create foreign table loc1_a_child (f1 serial, f2 text)
...
--Testcase 458:
create foreign table rem1_a_child (f1 serial, f2 text)
  server influxdb_svr options(table 'loc1');
--Testcase 717:
...
-- ===================================================================
--Testcase 466:
create foreign table gloc1_a_child (a int, b int generated always as (a * 2) stored)
  server influxdb_svr options(table 'gloc1');
--Testcase 719:
create table gloc1 (a int, b int generated always as (a * 2) stored, spdurl text) PARTITION BY LIST (spdurl);
--Testcase 720:
create foreign table gloc1_a PARTITION OF gloc1 FOR VALUES IN ('/node1/') SERVER spdsrv;
...
--Testcase 468:
insert into grem1_a_child (a) values (1), (22);
...
--Testcase 588:
create foreign table loct1_a_child (f1 int, f2 int, f3 int) server influxdb_svr options(table 'loct1');
--Testcase 724:
create table loct1 (f1 int, f2 int, f3 int, spdurl text) PARTITION BY LIST (spdurl);
--Testcase 725:
create foreign table loct1_a PARTITION OF loct1 FOR VALUES IN ('/node1/') SERVER spdsrv;
--Testcase 589:
create foreign table loct2_a_child (f1 int, f2 int, f3 int) server influxdb_svr options(table 'loct2');
--Testcase 726:
create table loct2 (f1 int, f2 int, f3 int, spdurl text) PARTITION BY LIST (spdurl);
--Testcase 727:
...
--Testcase 710:
CREATE FOREIGN TABLE ftable_a_child ( x int ) SERVER influxdb_svr OPTIONS ( table 'batch_table', batch_size '10' );
--Testcase 773:
...
--Testcase 724:
CREATE FOREIGN TABLE ftable_a_child ( x int ) SERVER influxdb_svr OPTIONS ( table 'batch_table', batch_size '1' );
--Testcase 779:
CREATE TABLE ftable ( x int, spdurl text) PARTITION BY LIST (spdurl);
--Testcase 780:
CREATE FOREIGN TABLE ftable_a PARTITION OF ftable FOR VALUES IN ('/node1/') SERVER spdsrv;
...
DROP FOREIGN TABLE ftable_a_child;
--Testcase 729:
DROP FOREIGN FROM batch_table;
--Testcase 785:
DROP TABLE ftable;

-- PARTITION BY HASH not suitable on PGSpider Extension
-- -- Use partitioning
-- --Testcase 730:
...
-- InfluxDB FDW does not support query cancel feature
